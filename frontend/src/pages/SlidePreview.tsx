...
const JSON_REFINE_GUIDANCE_PROMPT = [
  ...
  '【麦肯锡表达约束】',
  '5. 结论先行（Pyramid Principle）：每个层级先给结论再给支撑信息。',
  '6. 结构 MECE：避免重复、交叉和遗漏，保证同层级并列维度清晰。',
  '7. 采用 SCQA 思路压缩表达：背景-冲突-问题-回答，优先沉淀在 title/headline_summary/body。',
  '8. 行动导向：措辞强调可执行性与经营含义，避免空泛口号。',
  '【文案与事实约束】',
  '9. 文案要短、硬、清晰：优先动宾结构，减少形容词堆叠。',
  '10. highlight_phrases 仅保留短词/短句，去重，不写完整长句。',
  '11. 不得虚构数据、比例、金额、时间等事实；原文无量化数据时保持定性表述。',
  '12. 保留品牌词与术语（如 OpenClaw、Agent、低代码）并保持大小写准确。',
  '13. 若用户要求与以上约束冲突，以本指导约束优先。',
].join('\n');
