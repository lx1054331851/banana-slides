...
def test_147ai_adapter_keeps_gpt_image_2_variants_on_images_api():
    route = ResolvedProviderRoute(
        role="image",
        provider="openai",
        source="profile:147ai",
...
def test_gpt_image2_auto_mode_does_not_fallback_to_chat_on_5xx(monkeypatch):
    monkeypatch.setattr(
        "services.ai_providers.image.openai_provider.make_openai_client",
        lambda **_unused: object(),
    )
...
def test_gpt_image2_edit_auto_mode_does_not_fallback_to_chat_on_5xx(monkeypatch):
    monkeypatch.setattr(
        "services.ai_providers.image.openai_provider.make_openai_client",
        lambda **_unused: object(),
    )
    ...
    monkeypatch.setattr(provider, "_call_via_image_api_edits", _raise_image_api_error)
    monkeypatch.setattr(provider, "_call_via_chat_completions", _chat_should_not_run)

    with pytest.raises(Exception, match="Error generating image with OpenAI"):
        provider.generate_image(prompt="p", ref_images=[object()], aspect_ratio="16:9", resolution="4K")
    assert called["chat"] is False
