...
<b>Automatically extract charts from attachments, upload any materials, and request modifications verbally, moving towards true "Vibe PPT".</b>
...
Have you ever found yourself in this predicament: you have a presentation tomorrow, but your slides are still blank; your head is full of brilliant ideas, yet your enthusiasm is drained by tedious layout and design?
...
- 3️⃣ Similar visual output with severe homogenization
- 4️⃣ Low-quality assets that lack relevance
- 5️⃣ Fragmented layout of text and images, with poor design quality

These flaws make it difficult for traditional AI PPT generators to simultaneously satisfy our two major needs: "speed" and "aesthetics." Even those claiming to be "Vibe PPT" are, in my eyes, far from having enough "Vibe."
...
1. **Beginners**: Quickly generate aesthetically pleasing PPTs with zero barriers; no design experience required, reducing the hassle of selecting templates.
2. **PPT Professionals**: Rapidly gain design inspiration by referencing AI-generated layouts and combinations of graphics and text.
3. **Educators**: Swiftly convert teaching content into illustrated lesson plan PPTs to improve classroom engagement.
4. **Students**: Complete presentation assignments quickly, allowing them to focus on content rather than layout and aesthetics.
5. **Professionals**: Quickly visualize business proposals and product introductions with seamless adaptation across various scenarios.
...
View more <a href="https://github.com/Anionex/banana-slides/issues/2" > Use Cases </a>
...
- **Natural language editing**: Supports modifying outlines or descriptions via conversational "Vibes" (e.g., "Change page 3 to a case study"), with AI responding and adjusting in real-time.
...
- **Multi-format Support**: Upload PDF, Docx, MD, Txt, and other file formats for automatic background parsing.
...
- **In-painting**: Make verbal-style modifications to specific areas you are unsatisfied with (e.g., "Change this chart to a pie chart").
...
| Post-editing | Not supported | **Selection-based editing + Conversational editing** |
...
- 【2-9】：
  * New Features
    * Support for pasting images on the home page, outline, and description cards with immediate recognition and an improved interaction experience.
    ...
    * Internationalization + Dark Mode: Added English/Chinese switching; supports light/dark/follow system themes; full component adaptation for dark mode.
...
- 【1-4】 : v0.4.0 Release: Comprehensive upgrade to editable pptx export:
  * Supports maximum restoration of text styles such as font size, color, and bold from images;
  ...
  * **Detailed effects and usage can be found at https://github.com/Anionex/banana-slides/issues/121**

- 【12-27】: Added support for a template-less mode and high-quality text presets; PPT page styles can now be controlled through pure text descriptions.
...
| ✅ Completed | Selection-based Vibe verbal editing on individual PPT slides |
...
> **Friendly Reminder: The Google Nano Banana Pro model API has higher costs; please be mindful of usage expenses.**
```env
...
**Use the new version of the editable export configuration method to achieve better editable export results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter) and fill it in the `BAIDU_API_KEY` field in the `.env` file (there is ample free usage quota). For more details, see the instructions in https://github.com/Anionex/banana-slides/issues/121.


<details>
  <summary>📒 Vertex AI Configuration Guide (for GCP Users)</summary>

Google Cloud Vertex AI allows calling Gemini models through a GCP service account, and new users can use free credits. Configuration steps:
...
3. Set the following in `.env`:
   ...
   VERTEX_LOCATION=global
   ```
4. If using Docker for deployment, you also need to uncomment relevant sections in `docker-compose.yml` to mount the key file into the container and set the `GOOGLE_APPLICATION_CREDENTIALS` environment variable.
...
> **The LLM interfaces in this project follow the AIHubMix platform format. It is recommended to use [AIHubMix](https://aihubmix.com/?aff=17EC) to obtain API keys to minimize migration costs.** 
```env
...
<p align="left">
    <a href="https://github.com/zjunlp/DeepKE/blob/main/example/llm/README_CN.md">中文</a> | <a href="https://github.com/zjunlp/DeepKE/blob/main/example/llm/README.md">English</a>
</p>

<p align="left">
    <a href="https://github.com/zjunlp/DeepKE">
        <img src="https://img.shields.io/badge/DeepKE-v2.0-blue" alt="DeepKE">
    </a>
    <a href="https://github.com/zjunlp/DeepKE/blob/main/LICENSE">
        <img src="https://img.shields.io/badge/License-MIT-green" alt="License">
    </a>
</p>

DeepKE-LLM is a knowledge graph extraction and construction tool based on Large Language Models (LLMs). It supports various large models (such as the Llama series, ChatGLM series, etc.) and various extraction tasks (such as entity recognition, relation extraction, attribute extraction, etc.).
...
- [DeepKE-LLM](#deepke-llm)
  ...
  - [2. Quick Start](#2-quick-start)
    - [2.1 Model Deployment](#21-model-deployment)
    - [2.2 Starting the Web Interface](#22-starting-the-web-interface)
...
Configure the model path and related parameters in `config.py`:
...
### 2.2 Starting the Web Interface

Configure the backend API address in `frontend/.env`:
...
│   │   │   ├── DetailEditor.tsx     # Detail editor page
...
1. **Garbled or blurry text on generated pages**
    - You can select a higher output resolution (OpenAI format may not support increasing resolution; Gemini format is recommended). Based on testing, increasing the resolution from 1k to 2k before generating pages significantly improves text rendering quality.
...
A Commercial License is required for commercial use (e.g., closed-source use, private deployment and delivery, integrating this project into closed-source products, or providing services without disclosing the corresponding source code). Please contact the author: anionex@qq.com
...
<details>
  <summary>Thanks to <a href="https://api.chatfire.site/login?inviteCode=A15CD6A0">AI Fire</a> for sponsoring this project</summary>
  "A global multi-model API service aggregator. Enjoy secure, stable service with access to the latest global models within 72 hours at lower prices."
</details>
...
> If you have any questions about the sponsorship list, please feel free to <a href="mailto:anionex@qq.com">contact the author</a>
