...
<b>Go from idea to presentation in minutes, without tedious typesetting. Edit via natural language and move towards a true "Vibe PPT"</b>
...
Have you ever found yourself in this dilemma: you have a presentation tomorrow, but the PPT is still blank; your mind is full of brilliant ideas, but all your passion is drained by tedious layout and design?

We long to quickly create presentations that are both professional and well-designed. While traditional AI PPT generation apps mostly satisfy the need for "speed," they still suffer from the following issues:
...
- 3️⃣ Finished products look similar, with serious homogenization.
- 4️⃣ Low material quality and a lack of relevance.
- 5️⃣ Fragmented text-image layouts and poor design sense.

These flaws make it difficult for traditional AI PPT generators to simultaneously meet our two core needs: "speed" and "aesthetics." Even those claiming to be "Vibe PPT" are, in my eyes, far from truly having that "Vibe."

However, the emergence of the nano banana 🍌 model has changed everything. I tried using 🍌pro for PPT page generation and found the results to be exceptional in quality, aesthetics, and consistency. It can accurately render almost all text requested in the prompt and strictly follow the style of reference images. So, why not build a native "Vibe PPT" application based on 🍌pro?

## 👨‍💻 Applicable Scenarios

1. **Beginners**: Generate aesthetic PPTs quickly with zero barrier to entry, no design experience required, and less hassle in template selection.
...
4. **Students**: Quickly complete presentation assignments and focus energy on content rather than layout and beautification.
5. **Business Professionals**: Quickly visualize business proposals and product introductions with fast adaptation to various scenarios.
...
- **Natural Language Editing**: Supports modifying outlines or descriptions via natural language prompts (e.g., "Change page 3 to a case study"), with AI responding and adjusting in real-time.
...
No longer limited by complex menus, directly issue modification commands through **natural language**.
- **Local In-painting**: Perform verbal-style modifications on specific areas (e.g., "Change this chart to a pie chart").
...
| Re-editing | Edit via prompts | **Box-selection editing + Verbal editing** |
| Adding Assets | Cannot add after generation | **Freely add after generation** |
| Export Format | Supports PDF, (non-editable image) pptx | **Export to PDF, (image or editable) pptx** |
...
- [Feb 9]:
  * New Features
    * Added support for pasting images on the homepage, outline, and description cards for immediate recognition, providing a better interactive experience.
  ...
  * Fixes and Experience Optimization
    * Fixed export-related 500 errors, reference file association timing, outline/page data misalignment, incorrect project task polling, infinite polling in description generation, image preview memory leaks, and partial failure handling in batch deletion.
    * Optimized format example prompts, HTTP error message copy, Modal closing experience, cleaned up legacy project localStorage, and removed redundant prompts during first-time project creation.
    * Various other optimizations and fixes.
- [Jan 4]: v0.4.0 Release: Major upgrade to editable PPTX export:
  * Supports maximizing the restoration of font size, color, bold, and other styles of text within images;
  ...
  * Added page multi-select logic, allowing flexible selection of specific pages to generate and export.
...
This is the simplest method, with no need to install Docker or download the project. You can access the application directly after creation.

1. Deploy and start this application via Rainyun with one click (High bandwidth, suitable for HD image generation and downloads. New users get a 15-day free trial)
...
**(Optional, can also be configured in the user interface after startup)** Edit the `.env` file to configure the necessary environment variables:
> **The large model interfaces in this project follow the AIHubMix platform format as the standard. It is recommended to use [AIHubMix (click here to visit)](https://aihubmix.com/?aff=17EC) to obtain API keys to reduce migration costs.**<br>
> **Friendly Reminder: The interface fees for Google Nano and Banana Pro models are relatively high; please be mindful of usage costs.**
```env
...
**Use the new version of the editable export configuration method for better editable export results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter) and fill it in the `BAIDU_API_KEY` field in the `.env` file (there is a generous free usage quota). See the instructions in https://github.com/Anionex/banana-slides/issues/121 for details.


<details>
  <summary>📒 Vertex AI Configuration Guide (for GCP users)</summary>

Google Cloud Vertex AI allows calling Gemini models via GCP service accounts, and new users can use the free trial credits. Configuration steps:
...
3. Set the following in `.env`:
   ...
   VERTEX_LOCATION=global
   ```
4. If using Docker for deployment, you also need to uncomment relevant lines in `docker-compose.yml` to mount the key file into the container and set the `GOOGLE_APPLICATION_CREDENTIALS` environment variable.
...
> **The LLM interfaces in this project follow the AIHubMix platform format by default. It is recommended to use [AIHubMix](https://aihubmix.com/?aff=17EC) to obtain API keys and reduce migration costs.**
```env
...
1. **Garbled or blurry text on generated pages**
    - You can select a higher resolution output (the OpenAI format may not support increasing the resolution; using the Gemini format is recommended). Based on testing, adjusting the resolution from 1k to 2k before generating the page significantly improves text rendering quality.
    - Please ensure that the specific text content to be rendered is included in the page description.

2. **Poor quality when exporting editable PPT, such as overlapping text or missing styles**
...
4. **503 Error or Retry Error prompted when generating content**
...
This project is licensed under the **GNU Affero General Public License v3.0 (AGPL-3.0)**. It is freely available for non-commercial use, such as personal study, research, experimentation, education, or non-profit scientific research activities.
<details> 
<summary> Details </summary>
If you require a Commercial License (e.g., for closed-source use, private deployment delivery, integrating this project into closed-source products, or providing services without disclosing the corresponding source code), please contact the author: anionex@qq.com
...
<details>
  ...
  "Aggregating global multi-model API service providers. Enjoy secure, stable services and access to the world's latest models within 72 hours at a lower price."
</details>
